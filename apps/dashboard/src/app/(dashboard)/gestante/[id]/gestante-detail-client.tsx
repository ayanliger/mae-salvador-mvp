  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              {!g.ativa && (
                <Badge variant="outline" className="text-xs bg-slate-50 text-slate-500 border-slate-200">Concluída</Badge>
              )}
            </div>
            <p className="text-sm text-muted-foreground mt-0.5">
              {g.cpf ? `CPF ${g.cpf} · ` : ""}{g.idadeGestacionalSemanas} semanas{g.dpp ? ` · DPP ${fmt(g.dpp)}` : ""}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <Card>
          <CardContent className="pt-4 pb-3 px-4">
            ...
            <p className="text-xs text-muted-foreground">{g.dpp ? fmt(g.dpp) : "—"}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4 pb-3 px-4">
            <p className="text-xs text-muted-foreground">Consultas</p>
            <p className="text-lg font-bold">{consultasRealizadas}</p>
      ...
      <Tabs defaultValue="consultas" className="space-y-4">
          ...
          <div className="grid md:grid-cols-2 gap-4">
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <Row label="Endereço" value={
                  g.endereco.logradouro
                    ? `${g.endereco.logradouro}, ${g.endereco.numero} — ${g.endereco.bairro}`
                    : "—"
                } />
              </CardContent>
            </Card>
            <Card>
              ...
              <CardContent className="space-y-2 text-sm">
                ...
                <Row label="DUM" value={g.dum ? fmt(g.dum) : "—"} />
                <Row label="DPP" value={g.dpp ? fmt(g.dpp) : "—"} />
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  ...
                  {consultas.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Nenhuma consulta registrada.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  ...
                  {exames.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Nenhum exame registrado.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
                ...
                <TableBody>
                  {vacinas.map((v) => (
                    <TableRow key={v.id}>
                      ...
                      <TableCell className="text-sm">{v.dataPrevista ? fmt(v.dataPrevista) : "—"}</TableCell>
                  ...
                  {vacinas.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Nenhuma vacina registrada.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
            ...
            <CardContent className="p-0">
              <Table>
                ...
                <TableBody>
                  ...
                  {medicacoes.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-8 text-muted-foreground">Nenhuma medicação registrada.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
        ...
        {/* Visita à Maternidade — mock-tagged */}
